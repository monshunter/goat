...
func IsTargetFile(fileName string, excludes []string, skipNestedModules bool) bool {
	return IsTargetDir(filepath.Dir(fileName), excludes, skipNestedModules) && IsGoFile(fileName)
}

// IsTargetDir checks if the directory is a target directory
func IsTargetDir(dir string, excludes []string, skipNestedModules bool) bool {
	...
	if skipNestedModules && dir != "." && IsBelongtoNestedGoModule(dir) {
		return false
	}
...
// IsBelongtoNestedGoModule checks if a directory belongs to a nested Go module by searching upwards
// through the directory hierarchy until reaching the project root
func IsBelongtoNestedGoModule(dir string) bool {
	...
	for {
		...
		if !strings.HasPrefix(parentDir, projectRoot) {
			break
		}
