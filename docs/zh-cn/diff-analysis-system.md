...
1. **Repository Interface**：提供对Git仓库的访问并解析引用
2. **Diff Engines**：实现不同的算法来分析代码变更
3. **Output Structures**：用于表示文件和行变更的标准化数据结构
...
| 精度级别 | 小型仓库 | 中型仓库 | 大型仓库 |
|-----------------|------------------|-------------------|------------------|
|  1（高）  | 1-5秒      | 10-30秒     | 1-5分钟      |
|  2（中）| 0.1-0.5秒  | 1-3秒       | 5-30秒     |
|  3（低）   | 0.05-0.2秒 | 0.5-1秒     | 2-10秒     |
