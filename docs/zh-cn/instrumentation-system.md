...
1. **Template Engine**：使用Go模板生成追踪代码
2. **Values Builder**：构建模板渲染所需的数据
3. **AST Analysis**：使用抽象语法树分析Go代码结构
4. **Code Insertion**：在适当位置插入追踪代码
5. **Granularity System**：根据粒度级别确定插入点
6. **Runtime Components**：在运行时提供追踪和监控功能
